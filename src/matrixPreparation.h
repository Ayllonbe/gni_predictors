...
// Quick and dirty struct to store intermediate annotation information for
// computeSS
struct gene_anno {
    uint64_t gene = 0;
    const robin_hood::unordered_set<NetworKit::node>& annot;
    gene_anno(const uint64_t& g, const robin_hood::unordered_set<NetworKit::node>& a) :
    gene(g), annot(a) {}
};
...
// name - the GO name
// top - The GO namespace (biological_process, molecular_function or
// cellular_component) index - the node index in the created graph private ic -
...
public:
  ...
  GOTerm() {}
  GOTerm(std::string &id, std::string &name, std::string &top,
  ...
  void set_IC(double &val) { this->ic = val; }
  double get_IC() { return this->ic; }
  void set_child(NetworKit::node c) { this->childs.push_back(c); }
  NetworKit::node get_child(int pos) { return this->childs.at(pos); }
  std::vector<NetworKit::node> &get_childrens() { return this->childs; }
  void set_parent(NetworKit::node c) { this->parents.push_back(c); }
  NetworKit::node &get_parent(int pos) { return this->parents.at(pos); }
  std::vector<NetworKit::node> &get_parents() { return this->parents; }
...
void reader(
    ...
    robin_hood::unordered_map<std::string, Root> &namespace2root, int &threads);
...
public:
  ...
  void setAnnotation(NetworKit::node go,
                     robin_hood::unordered_set<std::string> &genes) {
  ...
  std::vector<std::string> getGenes(NetworKit::node &goTerm) {
    // BS: Only look up entry once in the hashmap
    auto ptr = this->go2gvec.find(goTerm);
    if (ptr != this->go2gvec.end()) {
        return ptr->second;
    }
  ...
  // BS: Get the number of genes and GO terms in the annotation data
  const uint64_t n_goterms() const {
    return this->go2gvec.size();
  }
  const uint64_t n_genes() const {
    return this->gene2govec.size();
  }
