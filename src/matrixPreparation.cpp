...
robin_hood::unordered_set<NetworKit::node> getancNodes( NetworKit::node& n ,robin_hood::unordered_map<NetworKit::node,GOTerm>& idx2goterm){
    ...
    for(NetworKit::node nd : idx2goterm.at(n).get_parents()){
        ...
        ancestors.insert(nd);
        robin_hood::unordered_set<NetworKit::node> vd = ::getancNodes(nd,idx2goterm);
...
void reader(std::string go_file, robin_hood::unordered_map<NetworKit::node,GOTerm>& idx2goterm,NetworKit::Graph& go, robin_hood::unordered_map<std::string,NetworKit::node>& gotermsID2idx, robin_hood::unordered_map<std::string,Root>& namespace2root, int& threads) {
    ...
    bool obsolete = false;
    robin_hood::unordered_map<NetworKit::node,std::vector<std::string>> node2parent;
    ...
    if (myfile.is_open())
    {
        ...
        robin_hood::unordered_map<NetworKit::node, std::vector<std::string>>::iterator it;
        ...
        robin_hood::unordered_map<NetworKit::node,GOTerm>::iterator iter=idx2goterm.begin();
...
arma::Mat<double> createPPIfromFile(std::string ppi_file, std::vector<std::string>& geneNodes){
    ...
    if (myfile.is_open())
    {
        ...
        arma::Mat<double> ppi(geneNodes.size(),geneNodes.size(),arma::fill::zeros);
 ...
 */
void readAnnotation(std::string fileAnnotation, Annotation& annotation, robin_hood::unordered_map<std::string, NetworKit::node>& go2idx, robin_hood::unordered_map<NetworKit::node,GOTerm>& idx2goterm, std::vector<NetworKit::node>& selectedGOs){
    std::ifstream myfile (fileAnnotation);
    std::string line;
    robin_hood::unordered_set<NetworKit::node> setGOs;

    robin_hood::unordered_map<std::string, robin_hood::unordered_set<NetworKit::node>> t2anc;
    ...
    robin_hood::unordered_map<std::string, robin_hood::unordered_set<std::string>> go2genes;
    ...
    if (myfile.is_open())
    {
        while ( getline (myfile,line) )
        {
            ...
            if(go2genes.find(results.at(1))==go2genes.end()){
                go2genes.insert({results.at(1),robin_hood::unordered_set<std::string>{}});
            ...
            if(go2idx.find(got)!=go2idx.end()){
                ...
                for(NetworKit::node n : t2anc.at(got))
                {
                    if(go2genes.find(idx2goterm.at(n).id)==go2genes.end()){
                        go2genes.insert({idx2goterm.at(n).id,robin_hood::unordered_set<std::string>{}});
                    }
                    go2genes.at(idx2goterm.at(n).id).insert(go2genes.at(got).begin(),go2genes.at(got).end());
                    setGOs.insert(n);
                }
            }

        }
        robin_hood::unordered_map<std::string, robin_hood::unordered_set<std::string>>::iterator git;
...
arma::Mat<double> computeGxTMatrix(std::vector<std::string>& geneNodes, std::vector<NetworKit::node>& selectedGOs, Annotation& annotation){
    arma::Mat<double> selAssoMatrix(geneNodes.size(),selectedGOs.size(),arma::fill::zeros);
...
double transitionalProb(NetworKit::node& s, NetworKit::node& t, Annotation& annotation, robin_hood::unordered_map<NetworKit::node, GOTerm>& idx2goTerm, double icSum){
...
double transitionalProbSUM(NetworKit::node& t, Annotation& annotation, robin_hood::unordered_map<NetworKit::node, GOTerm>& idx2goTerm, double icSum){
 ...
 */
arma::Mat<double> computeTPMatrix(Annotation& annotation, robin_hood::unordered_map<NetworKit::node, GOTerm>& idx2goTerm, std::vector<NetworKit::node>& selectedGOs){

    arma::Mat<double> TPMatrix(selectedGOs.size(),selectedGOs.size(),arma::fill::zeros);
    size_t t;
    ...
    for(t =0; t< selectedGOs.size(); t++){
        ...
        t2TPsum.insert({selectedGOs.at(t),transitionalProbSUM(selectedGOs.at(t),annotation,idx2goTerm,d)});
        t2ICchilds.insert({selectedGOs.at(t),d});
    }
    for(t =0; t< selectedGOs.size(); t++){

        for(NetworKit::node cn : idx2goTerm.at(selectedGOs.at(t)).get_childrens()){
            std::vector<NetworKit::node>::iterator it = find(selectedGOs.begin(),selectedGOs.end(),cn);
...
double TO(const robin_hood::unordered_set<NetworKit::node>& setGO1, const robin_hood::unordered_set<NetworKit::node>& setGO2, double& threshold){

    size_t ctr = 0; // Keep track of intersects
    for (const auto& item : setGO1) {
        if (setGO2.find(item) != setGO2.end()) {
            ctr++;
        }
    }

    double ss = static_cast<double>(ctr)/static_cast<double>((std::max(setGO1.size(),setGO2.size())));
    if(ss>=threshold){
        return ss ;
    }else{
        return 0.;
...
arma::Mat<double> computeSS(Annotation& annotation, std::vector<std::string>& geneNodes, int& threads, double& threshold){

    arma::Mat<double> ssMatrix(geneNodes.size(),geneNodes.size(),arma::fill::zeros);
    size_t i;
    size_t j;
...
void preComputeMatrix(int threads, double threshold_ss, std::string obofile, std::string goafile, std::string networkfile, std::string outFolder){
    ...
    std::cout <<"To read the network file took " <<duration.count()<<" seconds." << std::endl;
    //colvec c = sum(ppi,1);
    ...
    std::cout <<"To read the Gene Ontology took " <<duration.count()<<" seconds." << std::endl;
    // READ GOA file
    start = high_resolution_clock::now();
    ...
    stop = high_resolution_clock::now();
    duration = duration_cast<seconds>(stop - start);
    std::cout <<"To read the Annotation file took "  <<duration.count()<<" seconds." << std::endl;
    // Create association Matrix
    start = high_resolution_clock::now();
    arma::Mat<double> selAssoMatrix = computeGxTMatrix(geneNodes,selectedGOs, annotation);
    stop = high_resolution_clock::now();
    duration = duration_cast<seconds>(stop - start);
    std::cout << "To create the gene2GOterm association matrix took " <<duration.count()<<" seconds." << std::endl;
    // Create the transitional probability matrix
    start = high_resolution_clock::now();
    arma::Mat<double> TPMatrix =  computeTPMatrix(annotation,idx2goterm,selectedGOs);
    stop = high_resolution_clock::now();
    duration = duration_cast<seconds>(stop - start);
    std::cout << "To create the transitional probability matrix took "<< duration.count()<<" seconds." << std::endl;
    // Create the semantic ppi
    start = high_resolution_clock::now();

    arma::Mat<double> ssMatrix = computeSS(annotation, geneNodes,threads,threshold_ss);
    stop = high_resolution_clock::now();
    duration = duration_cast<seconds>(stop - start);
    std::cout <<"To create the semantic ppi took " <<duration.count()<<" seconds." << std::endl;
    std::cout <<"TESTF" <<std::endl;
    /*
    * NORMALIZE
    */
    /*       //PPI
       ...
       stop = high_resolution_clock::now();
       duration = duration_cast<seconds>(stop - start);
       cout <<"To normalize the PPI matrix took "<< duration.count()<<" seconds." << endl;
       // Semantic PPI
       start = high_resolution_clock::now();
...
int matrixPreparation(int ac, char* av[])
{
    ...
    desc.add_options()
            ...
            ("goFile,G",po::value<std::string>(&oboFile),"Set the path of gene ontology file [OBO format]")
            ("annotationFile,A",po::value<std::string>(&goaFile),"Set the path of annotation File [tsv format]")
            ("networkInput,N",po::value<std::string>(&networkFile), "network file")
            ("outFolder,O",po::value<std::string>(&outFolder)->default_value("./results"), "output folder path to provide the results")
            ;
