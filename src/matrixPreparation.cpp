...
computeTPMatrix(Annotation &annotation,
  ...
  for (t = 0; t < selectedGOs.size(); t++) {
    double d =
        icSum(idx2goTerm.at(selectedGOs.at(t)).get_childrens(), idx2goTerm);
    t2TPsum.insert(
        {selectedGOs.at(t),
...
arma::Mat<double> computeSS(Annotation &annotation,
  ...
  // the total number of annotation for a gene, so we need this to compute max
...
void preComputeMatrix(int threads, double threshold_ss, std::string obofile,
  ...
  /*       //PPI
     ...
     stop = high_resolution_clock::now();
     duration = duration_cast<seconds>(stop - start);
     cout <<"To normalize the PPI matrix took "<< duration.count()<<" seconds."
     << endl;
     // Semantic PPI
     start = high_resolution_clock::now();
     ...
     stop = high_resolution_clock::now();
     duration = duration_cast<seconds>(stop - start);
     cout <<"To normalize the semantic PPI matrix took "<< duration.count()<<"
     seconds." << endl;
